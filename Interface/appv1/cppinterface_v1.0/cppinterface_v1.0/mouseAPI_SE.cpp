...
MouseAPI::MouseData MouseAPI::Run()
{
	...
	GetDisplacement(&mdata.deltaX, &mdata.deltaY);
...
void MouseAPI::GetDisplacement(long *dx, long *dy)
{
	GetCursorPos(&pos);
	xDisplacement = pos.x - prevCoor.x;
	yDisplacement = pos.y - prevCoor.y;
	prevCoor.x = pos.x;
	prevCoor.y = pos.y;
	
	if (pos.x < xconsole) 
	{
		SetCursorPos((xconsole + dxconsole - 50), pos.y); //prevCoor.x = (xconsole + dxconsole - 50); 
	} 
	else if (pos.x >(xconsole + dxconsole)) 
	{
		SetCursorPos(xconsole + 50, pos.y); //prevCoor.x = xconsole + 50; 
	} 
	else if (pos.y < yconsole) 
	{
		SetCursorPos(pos.x, yconsole + dyconsole - 50);// prevCoor.y = yconsole + dyconsole - 50;
	} 
	else if (pos.y >(yconsole + dyconsole)) 
	{
		SetCursorPos(pos.x, yconsole + 50); //prevCoor.y = yconsole + 50; 
	} 
	if (std::abs(xDisplacement) > 10) xDisplacement = 0;
	if (std::abs(yDisplacement) > 10) yDisplacement = 0;
