...
public class UltrasoundImaging2forresults extends RoboticsAPIApplication {
	...
	File myLogfile = new File("UltrasoundImaging_results3_FFAN_9shahrivar1401.txt");//**this file will be saved in \\172.31.1.147\krc\ApplicationServer
														//User : kukauser
														//pass : 68kuka1secpw59
	FileWriter myWriter;
	...
	public void initialize() {
		...
		try
		{
			...
			myWriter = new FileWriter("UltrasoundImaging_results3_FFAN_9shahrivar1401.txt");
	...
	public void run() {
      ...
      //--parameters for y direction 
    	...
    	while (command != "STOP" && totalLoopTime < 10 )//everytime that a data is recieved over udp total loop time will be 0
        {
        	...
        	if (cmdvalues.length == 7)//only when data is received over udp, emgForce will get  updated
        	{
        		...
        		cmdDeltaX = Math.signum(Double.parseDouble(cmdvalues[0]))*0.1; //Double.parseDouble(cmdvalues[0])*0.1;//
        		cmdDeltaY = Math.signum(Double.parseDouble(cmdvalues[1]))*0.1;//Double.parseDouble(cmdvalues[1])*0.1;//
        		 rollangle =Double.parseDouble(cmdvalues[3]); 
              	 ...
              	 {
              		cmdDeltaX = Math.signum(cmdDeltaX) *  1;
              	 }
              	 
              	 if ( Math.abs(cmdDeltaY) > 10)
              	 {
              		cmdDeltaY = Math.signum(cmdDeltaY) *  1;
              	 }
      ...
      //----------------control over Force with constant impedance    	   
      	  ...
      	  // only excertion of emg force
          desiredCartesianPose[1] = (emgForce ) / stiffnessZ *1000 + currentCartesanPosition.getZ() ; //
